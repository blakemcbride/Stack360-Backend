 ...
 */
@Entity
@Table(name = "benefit_question_h")
public class BenefitQuestionH extends ArahantHistoryBean implements Serializable {
	...
	public BenefitQuestionH() {
		super();
	}

    @Override
    public String tableName() {
        return "benefit_question_h";
    }
	...
	public boolean alreadyThere() {
		...
		try
		{
			PreparedStatement stmt=ArahantSession.getHSU().getConnection().prepareStatement("select * from benefit_question_h where benefit_question_id=? and record_change_date=?");

			stmt.setString(1, benefitQuestionId);
