...
public class NewProjectInput extends TransmitInputBase {
	 ...
	 * @param bp
	 */
	public void makeProject(final BProject p) {


	//	p.setAllEmployees(isAccessibleToAll()?'Y':'N');
		
		p.setProjectStatusId(getProjectStatusId());
		...
		p.setRequestingOrgGroupId(getRequestingCompanyId());
		
	//	p.setEmployeeId(getEmployeeId());
		
	//	p.setSponsorId(getEmployeeId());
		
		if (getDateReported()==0)
			setDateReported(DateUtils.getDate(new Date()));
		
		p.setDateReported(getDateReported());

	//	p.setProductServiceId(getProductId());
	...
	//	p.setDollarCap(getDollarCap());
		...
		if (getReference()!=null && getReference().startsWith("ADX"))  //make sure ADX numbers get through
			p.setProjectName(getReference());
