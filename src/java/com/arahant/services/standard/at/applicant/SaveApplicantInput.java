...
//	private String []jobTypeIds;
    ...
    void setData(BApplicant bc) {
        ...
        for (int loop = 0; loop < getApplications().length; loop++) {
            if (applications[loop].getId() == null || applications[loop].getId().trim().equals(""))
                continue;

            ids.add(applications[loop].getId());
            BApplication app = new BApplication(applications[loop].getId());
            ...
            for (int loop2 = 0; loop2 < applications[loop].getContacts().length; loop2++) {
                if (applications[loop].getContacts()[loop2].getId() == null ||
                        applications[loop].getContacts()[loop2].getId().trim().equals(""))
                    continue;
                contactIds.add(applications[loop].getContacts()[loop2].getId());
                BApplicantContact con = new BApplicantContact(applications[loop].getContacts()[loop2].getId());
        ...
        for (int loop2 = 0; loop2 < getContacts().length; loop2++) {
            if (getContacts()[loop2].getId() == null ||
                ...
                continue;
            contactIds.add(getContacts()[loop2].getId());
        ...
        for (int loop = 0; loop < getQuestionDetails().length; loop++) {
            SaveApplicantInputQuestion q = getQuestionDetails()[loop];
