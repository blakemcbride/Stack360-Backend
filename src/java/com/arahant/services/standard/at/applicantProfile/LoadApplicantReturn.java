...
public class LoadApplicantReturn extends TransmitReturnBase {
    ...
    void setData(BApplicant bc) throws Exception {
        fname = bc.getFirstName();
        mname = bc.getMiddleName();
        lname = bc.getLastName();
        ...
        referredBy = bc.getReferredBy();
        veteran = bc.getVeteran() + "";
	...
	*/
        ...
        // We add 1 because when there is no job type, it is common.  [0] represent the common ones
        questionDetails = new LoadApplicantReturnQuestions[0];
        ...
        Record rec = db.fetchOne("select es.name " +
                ...
                bc.getPersonId(), DateUtils.today());
        employeeStatus = rec == null ? "" : rec.getString("name");
    }
