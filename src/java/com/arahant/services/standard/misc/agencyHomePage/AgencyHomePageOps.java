...
@WebService(targetNamespace="http://operations.arahant.com",serviceName="StandardMiscAgencyHomePageOps")
@SOAPBinding(style = Style.DOCUMENT, use = Use.LITERAL, parameterStyle = ParameterStyle.WRAPPED) 
@ThreadScope()
public class AgencyHomePageOps extends ServiceBase {

	private static final transient ArahantLogger logger = new ArahantLogger(
			AgencyHomePageOps.class);
	
	public AgencyHomePageOps() {
		super();
	}
	...
	*/
	...
	public SaveEmployeeReturn saveEmployee(/*@WebParam(name = "in")*/final SaveEmployeeInput in) {
		...
		try {
			...
			if (profileChanged(new BPerson(be.getPerson()), in)) {
				String message = emailChanges(new BPerson(be.getPerson()), in); //get the changes
				createProject(new BEmployee(be.getPersonId()), "Completed", "Completed", false, "Modify Employee: ", "Modify Employee: " + be.getNameFML(), message);
			} else {
				createProject(new BEmployee(be.getPersonId()), "Completed", "Completed", false, "Modify Employee: ", "Modify Employee: " + be.getNameFML(), "An Employee was modified in the system.");
			}
	...
	public ListEmployeeBenefitsReturn listEmployeeBenefits(/*@WebParam(name = "in")*/final ListEmployeeBenefitsInput in) {
		...
		try {
			...
			ret.setItem(new BEmployee(in.getEmployeeId()).getBenefitJoins());
    ...
    @WebMethod()
	public CancelBenefitReturn cancelBenefit(/*@WebParam(name = "in")*/final CancelBenefitInput in) {
		final CancelBenefitReturn ret = new CancelBenefitReturn();
		try {
			checkLogin(in);

			BHRBenefitJoin hrbj = new BHRBenefitJoin(in.getBenefitId());

			in.setData(hrbj);

			createProject(hrbj.getPayingPerson().getBEmployee(), "Completed", "Completed", false, "Cancel Employee Benefit", "Cancel Employee Benefit: " + hrbj.getPayingPerson().getBEmployee().getNameFML(), "A Benefit was Canceled for the Employee in the System.");
	...
	public InviteEmployeeToEnrollReturn inviteEmployeeToEnroll(/*@WebParam(name = "in")*/final InviteEmployeeToEnrollInput in) {
		...
		try {
			...
			createProject(new BEmployee(in.getEmployeeId()), "Initial", "Requested", true, "Benfit Enrollment", "Benefit Enrollment", "Please take the time to go to the Benefit Wizard and Enroll in any Benefits you may want.");
    ...
    @WebMethod()
	public ListAssignedGroupsForAgentReturn listAssignedGroupsForAgent(/*@WebParam(name = "in")*/final ListAssignedGroupsForAgentInput in)		
	{
		final ListAssignedGroupsForAgentReturn ret=new ListAssignedGroupsForAgentReturn();
		try
		{
			checkLogin(in);
			
			BAgent ba = new BAgent(hsu.getCurrentPerson().getPersonId());
			ret.setItem(ba.getAssignedCompanies(in.getShowActiveAuthorized(), in.getShowActive(), in.getShowInactive(), ba), ba);
    ...
    @WebMethod()
	public ListCompaniesForAgentReturn listCompaniesForAgent(/*@WebParam(name = "in")*/final ListCompaniesForAgentInput in)
	{
		final ListCompaniesForAgentReturn ret=new ListCompaniesForAgentReturn();
		try
		{
			checkLogin(in);

			ret.setItem(new BAgent(hsu.getCurrentPerson().getPersonId()).getActiveAgentCompanies());
    ...
    @WebMethod()
	public NewAgentMessageReturn newAgentMessage(/*@WebParam(name = "in")*/final NewAgentMessageInput in)		
	{
		final NewAgentMessageReturn ret=new NewAgentMessageReturn();
		try
		{
			checkLogin(in);

			BMessage.sendFromAgent(hsu.getCurrentPerson(), in.getMainContactIds(), in.getSubject(), in.getMessage());
    ...
    @WebMethod()
	public SubmitTransactionReturn submitTransaction(/*@WebParam(name = "in")*/final SubmitTransactionInput in)		
	{
		final SubmitTransactionReturn ret=new SubmitTransactionReturn();
		try
		{
			checkLogin(in);
			
			String outMessage = "Group Transaction Information:\n\nGroup: " + in.getGroup() + "\nEmployee: " + in.getEmployee() + "\nCarrier: " + in.getCarrier() + "\nTransaction Type: " + in.getTransactionType() + "\n";
