 ...
 *
 */
public class NewProcessScheduleInput extends TransmitInputBase {

	void setData(BProcessSchedule bc)
	{
	
		bc.setJavaClass(processId);
		
		
		//1-daily, 2-weekly, 3-monthly, 4-once, 5-system startup
		switch (type)
		{
			case 1: bc.setMinutes(DateUtils.getMinutes(dailyStartTime));
					bc.setHours(DateUtils.getHour(dailyStartTime));
					bc.setDaysOfMonth("*");
					bc.setMonths("*");
					bc.setDaysOfWeek("*");
					bc.setPerformMissingRuns(dailyPerformMissingRuns);
					bc.setStartDate(DateUtils.now());
					bc.setStartTime(DateUtils.nowTime());
					break;
			case 2: bc.setMinutes(DateUtils.getMinutes(weeklyStartTime));
					bc.setHours(DateUtils.getHour(weeklyStartTime));
					bc.setDaysOfMonth("*");
					bc.setMonths("*");
					bc.setDaysOfWeek("*");
					bc.setPerformMissingRuns(dailyPerformMissingRuns);
...
/*
		...
		bc.setWeeklyPerfor3mMissingRuns(weeklyPerformMissingRuns);
