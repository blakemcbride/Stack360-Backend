...
public class SearchLoginsReturn extends TransmitReturnBase {
	 ...
	 */
	public void setLogins(final FoundLogins[] logins) {
		this.logins = logins;
	}
	...
	void setLogins(final BPerson[] persons) {
		logins=new FoundLogins[persons.length];
		for (int loop=0;loop<persons.length;loop++)
			logins[loop]=new FoundLogins(persons[loop]);
	}
}
