...
public class SearchOrgGroupsReturn extends TransmitReturnBase {
	 ...
	 */
	public FoundOrgGroups[] getOrgGroups() {
		return orgGroups;
	}
	 ...
	 */
	public void setOrgGroups(final FoundOrgGroups[] orgGroups) {
	...
	void setOrgGroups(final BOrgGroup[] groups) {
		orgGroups=new FoundOrgGroups[groups.length];
		for (int loop=0;loop<groups.length;loop++)
			orgGroups[loop]=new FoundOrgGroups(groups[loop]);
	}
}
