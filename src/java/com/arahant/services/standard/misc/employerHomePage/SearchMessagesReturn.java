...
import com.arahant.services.standard.misc.agencyHomePage.MessageMessage;
...
public class SearchMessagesReturn extends TransmitReturnBase {
	...
	private MessageMessage[] messages;
	 ...
	 */
	public MessageMessage[] getMessages() {
		return messages;
	}
	 ...
	 */
	public void setMessages(final MessageMessage[] messages) {
		this.messages = messages;
	}

	void setMessages(final BMessage[] m) {
		messages=new MessageMessage[m.length];
		for (int loop=0;loop<m.length;loop++)
			messages[loop]=new MessageMessage(m[loop], false);
	}
}
