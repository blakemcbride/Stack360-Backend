...
public class ListFormTypesReturn extends TransmitReturnBase {
	...
	void setItem(final BFormType[] a) {
        ...
        for (int loop = 0; loop < a.length; loop++) {
            char ft = a[loop].getFormType();
            if (ft == 'E'  ||  ft == 'B')
                sz++;
        }
		item = new ListFormTypesReturnItem[sz];
		for (int loop = 0, i=0; loop < a.length; loop++) {
            char ft = a[loop].getFormType();
            if (ft == 'E'  ||  ft == 'B')
                item[i++] = new ListFormTypesReturnItem(a[loop]);
        }
	}
}
