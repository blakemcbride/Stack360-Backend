...
public class SearchScreenGroupsReturn extends TransmitReturnBase {
	...
	private ScreenGroup [] screenGroups;
	 ...
	 */
	public ScreenGroup[] getScreenGroups() {
		return screenGroups;
	}
	 ...
	 */
	public void setScreenGroups(final ScreenGroup[] screenGroups) {
	 ...
	 * @param groups
	 */
	void setScreenGroups(final BScreenGroup[] groups) {
		screenGroups=new ScreenGroup[groups.length];
		for (int loop=0;loop<groups.length;loop++)
			screenGroups[loop]=new ScreenGroup(groups[loop]);
	}

	void setScreenGroups(final BScreenOrGroup[] groups) {
		screenGroups=new ScreenGroup[groups.length];
		for (int loop=0;loop<groups.length;loop++)
			screenGroups[loop]=new ScreenGroup(groups[loop]);
	}
