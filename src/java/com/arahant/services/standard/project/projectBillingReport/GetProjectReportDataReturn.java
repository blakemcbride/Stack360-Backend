...
public class GetProjectReportDataReturn extends TransmitReturnBase {
    ...
    void makeProjectReportDetails(final HibernateScrollUtil bt, String sortOn, boolean sortAsc, final int max) {
        ...
        while (bt.next()) {
            ...
            final Double currentHours = (Double) elems[0];

            //second is a project
            Project p = (Project) elems[1];

            //third is billable state
            final Character billable = (Character) elems[2];
            ...
            if (billable == 'Y') {
                if (prd == null) {
                    ...
                    prd.setCurrentHours(currentHours);
                ...
                prd.setHours(currentHours.doubleValue(), billable.charValue(), p.getBillingRate());
        ...
        for (int loop = 0; loop < prda.length; loop++) {
            ...
            double remain = prda[loop].getEstimatedHours() - prda[loop].getInvoicedHours();
            double currentHours = prda[loop].getCurrentHours();
            //System.out.println("Current hours " + currentHours);

            if (remain < 0) {
                prda[loop].setRemainingEstimatedHours(0);
            }
            if (prda[loop].getEstimatedHours() == 0) {
                billableHours = currentHours;
            } else if (currentHours > remain) {
                billableHours = remain;
            } else {
                billableHours = currentHours;
            }
            ...
            prda[loop].setBillableHours(billableHours);
            billableHoursTotal += billableHours;
            dollars += billableHours * prda[loop].billingRate;
        ...
        setBillableHours(billableHoursTotal);
