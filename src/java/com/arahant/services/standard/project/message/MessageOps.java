...
@WebService(targetNamespace="http://operations.arahant.com",serviceName="StandardProjectMessageOps")
...
public class MessageOps extends ServiceBase {
	...
	public SearchPeopleReturn searchPeople(/*@WebParam(name = "in")*/final SearchPeopleInput in) {
		...
		try {
			...
			final String projectId = in.getProjectId();
			...
			if (shiftId == null || shiftId.isEmpty()) {
				query = "with shifts as (" +
						...
						"    join person " +
						"      on project_employee_join.person_id = person.person_id " +
						"    where project_shift_id in (select project_shift_id from shifts)) " +

						"select person_id from workers p " +
						"where 1=1 ";
				args.add(projectId);
			} else {
