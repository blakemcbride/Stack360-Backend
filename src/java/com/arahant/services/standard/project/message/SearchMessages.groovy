    ...
    static void main(JSONObject injson, JSONObject outjson, HibernateSessionUtil hsu, REST service) {
        ...
        if (shiftId == null || shiftId.isEmpty()) {
            stmt = "with shifts as (" +
                    ...
                    "    where project_shift_id in (select project_shift_id from shifts)) " +

                    "select m.message_id " +
                    "from message m " +
                    "left join message_to mt " +
                    "  on m.message_id = mt.message_id " +
                    "where (mt.to_person_id in (select person_id from workers) " +
                    "       or m.from_person_id in (select person_id from workers)) ";
            args.add(projectId)
        } else {
            stmt = "with workers as (" +
                    "    select person_id " +
                    "    from project_employee_join " +
                    ...
                    "where (mt.to_person_id in (select person_id from workers) " +
                    "       or m.from_person_id in (select person_id from workers)) " +
                    "       and m.from_person_id is not null ";
            args.add(shiftId)
        }
...
*/
        ...
        setMessages(outjson, results, includeBcc);
    }

    static void setMessages(final JSONObject outjson, final List<Record> mrecs, boolean includeBcc) {
        ...
        for (Record mrec : mrecs) {
            ...
            boolean handled = bm.getFromShow() == (char) 'N'
            ...
            for (Record rec : recs) {
                ...
                if (!handled)
                    handled = rec.getChar("to_show") == (char) 'N'
