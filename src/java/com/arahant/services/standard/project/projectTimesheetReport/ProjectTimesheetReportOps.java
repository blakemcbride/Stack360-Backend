...
@WebService(targetNamespace = "http://operations.arahant.com", serviceName = "StandardProjectProjectTimesheetReportOps")
@SOAPBinding(style = Style.DOCUMENT, use = Use.LITERAL, parameterStyle = ParameterStyle.WRAPPED)
@ThreadScope()
public class ProjectTimesheetReportOps extends ServiceBase {

	private static final ArahantLogger logger = new ArahantLogger(ProjectTimesheetReportOps.class);
	...
	@WebMethod()
	public SearchSubordinateEmployeesReturn searchSubordinateEmployees(/*
			 * @WebParam(name = "in")
			 */final SearchSubordinateEmployeesInput in) {
	...
	@WebMethod()
	public GetProjectReportDataReturn getProjectReportData(/*
			 * @WebParam(name = "in")
			 */final GetProjectReportDataInput in) {
		...
		try {
			...
			final HibernateScrollUtil sumAndProjects = BTimesheet.getTimesheetsForProjectReport(hsu, BPerson.getCurrent(),
					...
					in.getStartDate(), in.getEndDate(), in.isApproved(), in.isNotApproved(), in.isInvoiced(), 50);
	...
	@WebMethod()
	public SearchSubordinateOrgGroupsReturn searchSubordinateOrgGroups(/*
			 * @WebParam(name = "in")
			 */final SearchSubordinateOrgGroupsInput in) {
	...
	@WebMethod()
	public GetProjectReportReturn getProjectReport(/*
			 * @WebParam(name = "in")
			 */final GetProjectReportInput in) {
		final GetProjectReportReturn ret = new GetProjectReportReturn();

		try {
			checkLogin(in);

			ret.setReportUrl(new ProjectTimesheetReport().build(in.getClientCompanyId(), in.getEmployeeId(),
					in.getOrgGroupId(), in.getProjectId(), in.getStartDate(), in.getEndDate(),
					in.isApproved(), in.isNotApproved(), in.isInvoiced(), in.getShowInDollars()));
	 ...
	 * 2. Return projects accessible to all persons regardless of status 3.
	 * Return projects most recently used by some made up criteria
	 *
	 * Trickiness - on item 3, suppose you are taking 5 projects, that would be
	 * 5 projects that don't necessarily appear in 1 or 2 (so always 5 more if
	 * there are 5 more)?
	 */
	@WebMethod()
	 ...
	 * EmployeeEditorService.searchSubordinateEmployeesObj
	 * ProjectManagementService.getProjectReportDataObj
	 * ProjectManagementService.getProjectReportObj
	 * OrgGroupManagementService.searchSubordinateOrgGroupsObj
	 * ProjectManagementService.searchProjectsObj
