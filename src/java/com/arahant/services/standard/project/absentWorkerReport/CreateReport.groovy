...
package com.arahant.services.standard.project.absentWorkerReport
...
class CreateReport {
    ...
    static void main(JSONObject injson, JSONObject outjson, HibernateSessionUtil hsu, REST service) {
        ...
        Groff rpt = new Groff("Sub8WorkerReport-", "Sub-8 Worker Report", true)
 ...
 //       rpt.dontDeleteGroffFile()  //  for debugging
        ...
        recs.forEach(r -> {
            ...
            nts.forEach(r2 -> {
                ...
                Record ts = db.fetchOne("""
                                            select sum(total_hours) total_hours
                                            from timesheet 
                                            where project_id = ?
                                                  and person_id = ?
                                                  and beginning_date = ?
                ...
                Double hours = ts == null ? 0.0 : ts.getDouble("total_hours")
                String sh = hours == null || hours < 0.1 ? "0" : NumberFormat.Format(hours, "", 0, 2)
                if (hours < 7.99) {
                    ...
                    String eventS = BHREmployeeEvent.getMobileEventString(db, r2.getString("person_id"), workDate)
                    ...
                    rpt.out "\t" + StringUtils.take(name + " (" + sh + ") " + phone + " " + eventS, 140)
                }
            })
            if (!nts.isEmpty())
                rpt.out(".sp")
    ...
    private static void page_title(Groff rpt, int workDate, String client, String subtype, Connection db) {
        rpt.out(".SP")
        rpt.out("Workers who worked less than 8 hours for work date:  " + DateUtils.format("EEE MMM d, yyyy", workDate))
