...
class SaveConfirmation {
    ...
    static void main(JSONObject injson, JSONObject outjson, HibernateSessionUtil hsu, REST service) {
        ...
        boolean confirmed = injson.getBoolean('confirmed')
        ...
        for (Record rec : recs) {
            if (confirmed) {
                rec.setDateTime("confirmed_date", new Date())
                rec.set("confirmed_person_id", hsu.getCurrentPerson().getPersonId())
            } else {
                rec.setDateTime("confirmed_date", null)
                rec.set("confirmed_person_id", null)
            }
            rec.update()
        }
    }
