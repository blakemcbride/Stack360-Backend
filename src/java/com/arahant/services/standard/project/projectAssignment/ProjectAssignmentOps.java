...
@WebService(targetNamespace = "http://operations.arahant.com", serviceName = "StandardProjectProjectAssignmentOps")
@SOAPBinding(style = Style.DOCUMENT, use = Use.LITERAL, parameterStyle = ParameterStyle.WRAPPED)
@ThreadScope()
public class ProjectAssignmentOps extends ServiceBase {

    static ArahantLogger logger = new ArahantLogger(ProjectAssignmentOps.class);
                ...
                for (AssignedWorkers aw : assigned) {
                    ...
                    if (pejId == null || pejId.isEmpty()) {
                        ...
                        if (pej == null) {
                            ...
                            rec.set("person_id", person.getPersonId());
                            rec.set("project_shift_id", newShift.getProjectShiftId());
                            rec.set("change_person_id", hsu.getCurrentPerson().getPersonId());
                            rec.set("change_date", org.kissweb.DateUtils.today());
                        ...
                        if (pej != null)
                            if (!alreadyExists) {
                                ...
                                pej.setProjectShift(newShift);
                                pej.setStartDate(aw.getStartDate());
                                pej.setHours(aw.isReportsHours() ? "Y" : "N");
                                pej.setManager(aw.isManager() ? "Y" : "N");
                                hsu.update(pej);
                            } else {
            ...
            if (unassignedItems != null)
                for (UnassignedItem ui : unassignedItems) {
                    ...
                    if (pej != null) {
                        Record rec = db.newRecord("project_employee_history");
                        IDGenerator.generate(rec, "project_employee_history_id");
                        rec.set("person_id", pej.getPerson().getPersonId());
                        rec.set("project_shift_id", pej.getProjectShiftId());
                        rec.set("change_person_id", hsu.getCurrentPerson().getPersonId());
                        rec.set("change_date", org.kissweb.DateUtils.today());
                        rec.set("change_time", TimeUtils.now());
...
/*
        Set<UnassignedItem> oldUnassignedItems = new HashSet<>();
        boolean oldActive = bp.getActive();
            ...
            for (Person per : bp.getAssignedPersons2(in.getShiftId()))
            ...
            if (in.getUnassignedItems() != null)
                for (UnassignedItem unassignedItem : oldUnassignedItems) {
...
//				in.setOrgGroupId(in.getCompanyId());
    ...
    public SaveProjectForPersonReturn saveProjectForPerson(/*@WebParam(name = "in")*/final SaveProjectForPersonInput in) {
        ...
        try {
            ...
            hcu.joinTo(ProjectEmployeeJoin.PROJECTSHIFT)
                    .joinTo(ProjectShift.PROJECT)
                    .eq(Project.PROJECTID, in.getProjectId());
            hcu.joinTo(ProjectEmployeeJoin.PERSON)
