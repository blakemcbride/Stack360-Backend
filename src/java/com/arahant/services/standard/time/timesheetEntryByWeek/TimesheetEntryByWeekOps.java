...
@WebService(targetNamespace="http://operations.arahant.com",serviceName="StandardTimeTimesheetEntryByWeekOps")
@SOAPBinding(style = Style.DOCUMENT, use = Use.LITERAL, parameterStyle = ParameterStyle.WRAPPED) 
@ThreadScope()
public class TimesheetEntryByWeekOps extends ServiceBase {

	private static final transient ArahantLogger logger = new ArahantLogger(
			TimesheetEntryByWeekOps.class);
	
	public TimesheetEntryByWeekOps() {
		super();
	}
	...
	public AddTimeEntryReturn addTimeEntry(/*@WebParam(name = "in")*/final AddTimeEntryInput in)	{
		...
		try {
			...
			final String benefitId=bemp.newTimeEntry(in.getBeginningTime(),
					...
					in.getProjectId(), null, in.getTotalHours(),in.getWorkDate(), -1);
	...
	public ListTimesheetsForPersonOnDateReturn listTimesheetsForPersonOnDate(/*@WebParam(name = "in")*/final ListTimesheetsForPersonOnDateInput in)	{
		...
		try {
			...
			ts=e.listTimesheetsForWeek(in.getTimesheetDate());
	...
	public SaveTimeEntryReturn saveTimeEntry(/*@WebParam(name = "in")*/final SaveTimeEntryInput in)	{
		...
		try {
			...
			final String benefitId=bemp.saveTimeEntry(in.getBeginningTime(),
					...
					in.getProjectId(),in.getTotalHours(),in.getWorkDate(), -1, in.getTimesheetId());
    ...
    @WebMethod()
	public GetNextWeekReturn getNextWeek(/*@WebParam(name = "in")*/final GetNextWeekInput in)		
	{
		final GetNextWeekReturn ret=new GetNextWeekReturn();
		try
		{
			checkLogin(in);
			
			ret.setData(in.getStartDate(), in.getForward());
