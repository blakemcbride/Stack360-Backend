...
public class TimesheetTransmit {
	...
	TimesheetTransmit(final BTimesheet t) {
		...
		dayOfWeek = DateUtils.getDayOfWeekAbbreviated(workDate);
		
		beginTimeFormatted = DateUtils.getDateFormatted(workDate) + " " + DateUtils.getTimeFormatted(beginningTime);
		endTimeFormatted = DateUtils.getDateFormatted(t.getEndDate()) + " " + DateUtils.getTimeFormatted(endTime);
	}
