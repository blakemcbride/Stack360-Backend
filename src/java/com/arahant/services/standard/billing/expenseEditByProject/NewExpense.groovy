...
class NewExpense {
    ...
    static void main(JSONObject injson, JSONObject outjson, HibernateSessionUtil hsu, REST service) {
        BExpense bexp = new BExpense()
        bexp.create()
        Expense exp = bexp.getExpense();
        exp.setEmployee((new BEmployee(injson.getString("employee_id"))).getEmployee())
        ...
        exp.setSchedulingComments(injson.getString('scheduling_comments'))
        exp.setPerDiemReturn(injson.getFloat('per_diem_return'))
        exp.setPaymentMethod(injson.getString('payment_method').charAt(0))
        exp.setAuthDate(DateUtils.today())
        exp.setAuthPerson(hsu.getCurrentPerson())
        exp.setWeekPaidFor(injson.getInt("week_paid_for"))
        bexp.insert()
    }
