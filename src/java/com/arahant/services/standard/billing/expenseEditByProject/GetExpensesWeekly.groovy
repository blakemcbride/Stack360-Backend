 ...
 * Date: 3/3/18
 */
class GetExpensesWeekly {

    static void main(JSONObject injson, JSONObject outjson, HibernateSessionUtil hsu, REST service) {
        String project_id = injson.getString("project_id")
        int project_week = injson.getInt("project_week")
        ...
        HibernateCriteriaUtil<Expense> hcu = hsu.createCriteria(Expense.class).eq(Expense.PROJECT_ID, project_id)
                .ge(Expense.WEEK_PAID_FOR, project_week)
                .lt(Expense.WEEK_PAID_FOR, DateUtils.add(project_week, 7))
