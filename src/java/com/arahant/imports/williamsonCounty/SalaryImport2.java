...
public class SalaryImport2 {
	...
	public static void doImport(String path) {
		...
		try {
			...
			DelimitedFileReader dfr = new DelimitedFileReader(path);
			...
			while (dfr.nextLine())
			{
				...
				//System.out.println(dfr.getString(3));

				//System.out.print("\'" + ref + "\'" + ", ");

				if(!StringUtils.isEmpty(dfr.getString(3)))
				{
					String ref = dfr.getString(0);

					double sal = roundTwoDecimals(MoneyUtils.parseMoney(dfr.getString(3)));
				
					Employee emp = hsu.createCriteria(Employee.class).eq(Employee.SSN, ref).first();

					if (emp == null)
					{
						System.out.println("Missing ref " + ref + " " + dfr.getString(0) + " " + dfr.getString(1) + " " + dfr.getString(2));
						continue;
					}
