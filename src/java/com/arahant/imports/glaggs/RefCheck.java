 ...
 *
 */
public class RefCheck extends GLAGSFixedImportBase {
...
//fr.printCurrentLine();
		...
		//		continue;
			...
			System.out.println(name+" = "+secId);

			CompanyDetail cdt=hsu.createCriteria(CompanyDetail.class)
				.eq(CompanyDetail.NAME,name)
				.first();
			...
			{
				secId=(fr.getField("GroupNumber")+fr.getField("GroupSuffix")).trim();
			}

			cdt.setExternalId(secId);
			hsu.saveOrUpdate(cdt);
		}
