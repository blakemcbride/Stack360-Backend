 ...
 * com.arahant.dynamic.services.custom.waytogo.employeeExport
 * 
 */
class WayToGoEmployeeExport {
	...
	private spinClients() {
		...
		while (scr.next()) {
			...
			Project [] proj = new Project[4]
			List<ProjectEmployeeJoin> pejl = hsu.createCriteria(ProjectEmployeeJoin.class).eq(ProjectEmployeeJoin.PERSON, emp).list()
			int j = 0
			for (ProjectEmployeeJoin pej : pejl)
				if (j < 4)
					proj[j++] = pej.getProject()
			if (proj[0] != null)
				dfw.writeField proj[0].getReference()
			else
				dfw.writeField ""
			if (proj[1] != null)
				dfw.writeField proj[1].getReference()
			else
				dfw.writeField ""
			if (proj[2] != null)
				dfw.writeField proj[2].getReference()
			else
				dfw.writeField ""
			if (proj[3] != null)
				dfw.writeField proj[3].getReference()
			else
				dfw.writeField ""
			...
			if (orgGroups.isEmpty()) {
				...
				OrgGroupAssociation [] oga = orgGroups.toArray(new OrgGroupAssociation[orgGroup.size()])
    ...
    private void writeColumnHeader() {
		...
		dfw.writeField "Shift 1"
		dfw.writeField "Shift 2"
		dfw.writeField "Shift 3"
		dfw.writeField "Shift 4"
