...
class P_3989 {
    ...
    private static final ArahantLogger logger = new ArahantLogger(P_3989.class)

    public static run(Connection db) {
        logger.info("Updating project_form table")
        Command cmd = db.newCommand()
        Cursor c = cmd.query("select * from project_form")
        int i = 1;
        while (c.isNext()) {
