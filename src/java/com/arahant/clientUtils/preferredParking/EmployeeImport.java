...
/*
 */
package com.arahant.clientUtils.preferredParking;
...
import static com.arahant.clientUtils.preferredParking.EmployeeImport.ColumnName.*;
 ...
 */
public class EmployeeImport extends GenericImport {
	...
	public EmployeeImport(String filename, int mode) throws FileNotFoundException {
			...
			for (ColumnInfo ci : ecols) {
				...
				if (ci.equals(SSN)) {
					...
					if (fld.length() > 0) {
						...
						if (fld.length() > 0) {
							String id="";
							if ("Employee Screens".equalsIgnoreCase(fld))
								id = IDGenerator.expandKey("1-158");
							else if ("Manager Screens".equalsIgnoreCase(fld))
								id = IDGenerator.expandKey("1-216");
						...
						if (fld.length() > 0) {
							String id="";
							if ("All Employees".equalsIgnoreCase(fld))
								id = IDGenerator.expandKey("0-0");
							else if ("Managers".equalsIgnoreCase(fld))
								id = IDGenerator.expandKey("0-2");
							if (null == hsu.get(SecurityGroup.class, id))
...
//				throw ioe;
			...
			for (ColumnInfo ci : ecols) {
				...
				try {
					if (ci.equals(SSN)) {
						...
						if (fld.length() > 0)
							be.setUserPassword(password=fld, true, true);
					} else if (ci.equals(SCREEN_GROUP)) {
						if (fld.length() > 0) {
							String id="";
							if ("Employee Screens".equalsIgnoreCase(fld))
								...
								id = IDGenerator.expandKey("1-216");
							be.setScreenGroupId(screen_group=id);
						}
						...
						if (fld.length() > 0) {
							...
							if ("All Employees".equalsIgnoreCase(fld))
								...
								id = IDGenerator.expandKey("0-2");
							be.setSecurityGroupId(security_group=id);
						}
					} else if (ci.equals(EMPLOYMENT_STATUS)) {
			...
			// set loginID and password to both be last name initial and last four of SSN
			int sslen = ssn.length();
			password=login_id = be.getLastName().charAt(0) + ssn.substring(sslen-4, sslen);
			be.setUserLogin(login_id);
			be.setUserPassword(password, true, true);
...
//				be.insert();
			...
			if (!existingEmployee  &&  employeeStatus != null  &&  employeeStatusDate > 0) {
			...
			if (!existingEmployee  &&  hireStatus != null  &&  hireDate > 0  &&  hireDate != employeeStatusDate) {
				BHREmplStatusHistory bsh = new BHREmplStatusHistory();
				bsh.create();
			...
			if (!existingEmployee  &&  payType != ' '  &&  payAmount > 0.01  &&  position != null) {
...
//		HrEmployeeStatus inactiveStatus = hsu.createCriteria(HrEmployeeStatus.class).eq(HrEmployeeStatus.ACTIVE, 'N').first();
	...
	public static void main(String [] argv) {
		try {
			...
			String reportFile = new EmployeeImport("/home/blake/Arahant/Clients/PreferredParking/PPS HR Census 9-01-14.csv", UPDATE_MODE).parse();
