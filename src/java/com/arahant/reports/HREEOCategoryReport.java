...
public class HREEOCategoryReport extends PdfPageEventHelper {

	private Font baseFont;
	private Document document;
	private HibernateSessionUtil hsu;

	public HREEOCategoryReport() {
	...
	protected void writeHeader() throws DocumentException {
		...
		cell = new PdfPCell(new Paragraph("EEO Categories Report", new Font(FontFamily.COURIER, 12F, Font.NORMAL)));
	...
	protected void writeEEOCategories() throws DocumentException {
		PdfPTable table;
		PdfPCell cell;
		boolean alternateRow = true;
		final BaseColor alternateRowColor = new BaseColor(192, 192, 192);
		final HibernateCriteriaUtil hcu = this.hsu.createCriteria(HrEeoCategory.class).orderBy(HrEeoCategory.NAME);
		final List eeoCategories = hcu.list();
		final Iterator iterator = eeoCategories.iterator();
		HrEeoCategory eeoCategory;
		...
		cell = new PdfPCell(new Paragraph("Category", this.baseFont));
		...
		while (iterator.hasNext()) {
			eeoCategory = (HrEeoCategory) iterator.next();

			// toggle the alternate row
			alternateRow = !alternateRow;

			cell = new PdfPCell(new Paragraph(eeoCategory.getName(), this.baseFont));
			if (alternateRow)
