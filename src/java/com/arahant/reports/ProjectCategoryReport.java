 ...
 *
 */
public class ProjectCategoryReport extends PdfPageEventHelper {
	private Font baseFont;
	private Document document;
	private HibernateSessionUtil hsu;
    
    public ProjectCategoryReport() {
	...
	protected void writeHeader() throws DocumentException {
        ...
        cell = new PdfPCell(new Paragraph("Project Category Report", new Font(FontFamily.COURIER, 12F, Font.NORMAL)));
	...
	protected void writeCategories() throws DocumentException {
		PdfPTable table;
        PdfPCell cell;
		boolean alternateRow = true;
		final BaseColor alternateRowColor = new BaseColor(192, 192, 192);
		final HibernateCriteriaUtil hcu = this.hsu.createCriteria(ProjectCategory.class).orderBy(ProjectCategory.CODE);
		final List categories = hcu.list();
        ...
        while (iterator.hasNext()) {
        	category = (ProjectCategory)iterator.next();
        		
        	// toggle the alternate row
			alternateRow = !alternateRow;
            
            cell = new PdfPCell(new Paragraph(category.getCode(), this.baseFont));
            if (alternateRow)
				cell.setBackgroundColor(alternateRowColor);
            cell.disableBorderSide(1 | 2 | 4 | 8);
            table.addCell(cell);
            
            cell = new PdfPCell(new Paragraph(category.getDescription(), this.baseFont));
            if (alternateRow)
				cell.setBackgroundColor(alternateRowColor);
            cell.disableBorderSide(1 | 2 | 4 | 8);
            table.addCell(cell);
