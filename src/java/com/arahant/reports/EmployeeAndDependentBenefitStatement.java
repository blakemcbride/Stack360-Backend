 ...
 *
 */
public class EmployeeAndDependentBenefitStatement extends ReportBase {
	 ...
	 */
	public EmployeeAndDependentBenefitStatement() throws ArahantException {
	...
	private Image logo;//= readImage("report-logo.gif", 20, 20);
	...
	final protected void writeEmployee(BPerson emp, boolean includeCredentials, int date) throws DocumentException, ArahantException {
...
//		now do it for each dependent
		...
		if (emp.isEmployee()) {
			...
			for (BHREmplDependent dep : bemp.getDependents()) {
				...
				dphcu.eq(HrBenefitJoin.APPROVED, 'Y');
				...
				List<HrBenefitJoin> dpList = dphcu.list();
				depInfo.list = dpList;
//				while (dpScroll.next()) {
...
//				dpScroll.close();
			...
			writeBody(categoryToBenefitInfo, uncategorized, depList);
//			writeDependentInfo(depList);
	 ...
	 *
	 */
	private void writeBody(HashMap<String, List<BenefitInfo>> categoryToBenefitInfo, List<BenefitInfo> uncategorized, List<DepInfo> depList) throws DocumentException {

		PdfPTable table = makeTable(new int[]{50, 15, 20, 25});
...
//			write out bene name and stuff
		...
		writeColHeaderBold(table, "Benefit : Coverage Configuration", 11F);
		...
		for (OutputData od : odList) {
			writeLeft(table, od.name, true);
			writeRight(table, od.hasIt, true);
			writeAlign(table, od.date, Element.ALIGN_CENTER, true);
			writeLeft(table, od.providedBy, true);
			addTable(table);
			table.flushContent();

			if (depList != null && !depList.isEmpty())
				writeDependentInfo(depList, od.name.split(":")[0].trim());
		}
...
//		addTable(table);
	 ...
	 * table; }
	 */
	private void writeDependentInfo(List<DepInfo> depList, String beneName) throws DocumentException {

		PdfPTable table = makeTable(new int[]{10, 10, 20, 25, 20, 25});
//
//		writeColHeaderBold(table, "Dependent Benefits and Effective Dates", 11F);

//		addTable(table);

//		int[] cols = new int[]{10, 20, 20, 20, 20,20};
...
//		}
//		/*      if (showLTC) {
//		writeCenteredNoBorder(table, "Long Term Care");
//		writeCenteredNoBorder(table, "");
//		} else
//		 */		{
//			writeCenteredNoBorder(table, "");
...
//		table.setHeaderRows(1);
		...
		for (DepInfo depInfo : depList) {
			...
			for (HrBenefitJoin j : depInfo.list)
				if (new BHRBenefitJoin(j).getBenefitName().equals(beneName)) {
					hasBene = true;
					bj = j;
					break;
				}
			...
			if (hasBene) {
				writeAlign(table, "");
				writeAlign(table, depInfo.lname.trim() + ", " + depInfo.name.trim(), Element.ALIGN_LEFT, false, 3);
				writeAlign(table, DateUtils.getDateFormatted(bj.getCoverageStartDate()), Element.ALIGN_CENTER, false);
				writeAlign(table, "");
			}
		}
//		for (DepInfo depInfo : depList) {
//			/*   if (depInfo.careDate == 0 && depInfo.dentDate == 0 && depInfo.medDate == 0 && depInfo.visDate == 0) {
//			continue;
//			}
//			 */
//			//logger.info(depInfo);
...
//		}
	...
	private void writeDependentInfo(PdfPTable table, DepInfo depInfo) {
		if (depInfo.careDate == 0 && depInfo.dentDate == 0 && depInfo.medDate == 0 && depInfo.visDate == 0)
			return;

		//logger.info(depInfo);
		writeCenteredNoBorder(table, depInfo.name);
		writeCenteredNoBorder(table, DateUtils.getDateFormatted(depInfo.bday));
		writeCenteredNoBorder(table, depInfo.medDate == 0 ? "No" : DateUtils.getDateFormatted(depInfo.medDate));
		writeCenteredNoBorder(table, depInfo.dentDate == 0 ? "No" : DateUtils.getDateFormatted(depInfo.dentDate));
		writeCenteredNoBorder(table, depInfo.visDate == 0 ? "No" : DateUtils.getDateFormatted(depInfo.visDate));
...
//		writeBoldCentered(table, BProperty.get("Announcement"), 16F);
	...
	public static void main(String args[]) {
		try {
			...
			EmployeeAndDependentBenefitStatement ebj = new EmployeeAndDependentBenefitStatement();
