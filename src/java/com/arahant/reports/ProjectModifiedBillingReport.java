...
public class ProjectModifiedBillingReport extends ReportBase {
    ...
    public ProjectModifiedBillingReport() {
        super("prmodbillrpt", "Project Billing Report", false);
    }
...
//            }
        ...
        try {
            ...
            addColumnHeader("Client", Element.ALIGN_LEFT, 20);
            addColumnHeader("ID", Element.ALIGN_LEFT, 5);
            addColumnHeader("Project Summary", Element.ALIGN_LEFT, 60);
...
//            addColumnHeader("Hours Remaining", Element.ALIGN_RIGHT, 8);
            addColumnHeader("Hours", Element.ALIGN_RIGHT, 15);
//            addColumnHeader("Billable", Element.ALIGN_RIGHT, 8);
//            addColumnHeader("Total", Element.ALIGN_RIGHT, 15);
    ...
    private void printReport(ArrayList<ReportDetail> reportDetails, PdfPTable table) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            double totalBill = rd.getCurrent();
...
//            writeRight(table, Formatting.formatNumberWithCommas(rd.getRemaining(), 0) + "", false);
            writeRight(table, Formatting.formatNumberWithCommas(rd.getCurrent(), 2) + "", false);
...
//        write(table, "", true);
        writeRight(table, "Totals", true);
//        writeRight(table, Formatting.formatNumberWithCommas(totalRemaining, 0) + "", true);
        writeRight(table, Formatting.formatNumberWithCommas(totalCurrent, 2) + "", true);
 //       writeRight(table, Formatting.formatNumberWithCommas(totalBillable, 0) + "", true);
 //       writeRight(table, MoneyUtils.formatMoney(totalAmountBilledTotal) + "", true);
...
//            writeRight(table, Formatting.formatNumberWithCommas(grandTotalRemaining, 0) + "", true);
            writeRight(table, Formatting.formatNumberWithCommas(grandTotalCurrent, 2) + "", true);
...
//            writeRight(table, MoneyUtils.formatMoney(grandTotalAmountBilledTotal) + "", true);
            ...
            Logger.getLogger(ProjectModifiedBillingReport.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public static void main(String[] args) {
        ...
        ArahantSession.getHSU().setCurrentPerson(new BPerson("00000-0000000000").getPerson());
        try {
            new ProjectModifiedBillingReport().build("", "", "", "", true, true, false);
        } catch (DocumentException ex) {
            Logger.getLogger(ProjectModifiedBillingReport.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
