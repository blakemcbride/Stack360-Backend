 ...
 *
 */
public class HREvaluationReport extends ReportBase {
	
	private HibernateSessionUtil hsu;
    
    public HREvaluationReport() throws ArahantException {
    	super("EvalRpt","Employee Evaluation Report");
    }

	public String build(final HibernateSessionUtil hsu, final String employeeId, final BHREmployeeEval[] employeeEvals, final boolean showPrivate) throws FileNotFoundException, DocumentException {
        this.hsu = hsu;
        ...
        try {
            ...
            this.writeEvaluation(employeeEvals,showPrivate);
        } finally {
            close();
        }
        return getFilename();
	...
	protected void writeEvaluation(final BHREmployeeEval[] employeeEvals, final boolean showPrivate) throws DocumentException {
		PdfPTable table;
        PdfPCell cell;
		...
		for (final BHREmployeeEval element : employeeEvals) {	
            ...
            supervisor = employeeEval.getSupervisorLName() + ", " + employeeEval.getSupervisorFName();
...
//		  write out employee comments
		    if (showPrivate)
		    {
			    ...
			    cell = new PdfPCell(new Paragraph("Internal Comments", this.baseFont));
			    cell.disableBorderSide(1 | 4 | 8);
			    table.addCell(cell);
			    table.setHeaderRows(1);
			    
			    write(table,employeeEval.getInternalSupervisorComments());
            ...
            try {
        		...
        		if (showPrivate)
        			table = makeTable(new int[] {16, 16, 16, 16, 16, 16 });
        		else
    		    ...
    		    if (showPrivate)
    		    {
	    		    cell = new PdfPCell(new Paragraph("Int Notes", this.baseFont));
	    		    cell.disableBorderSide(1 | 4 | 8);
	    		    table.addCell(cell);
    		    }
    		    table.setHeaderRows(1);
            	...
            	for (final BHREmployeeEvalDetail element0 : employeeEvalDetails) {
        			...
        			if (showPrivate)
        				write(table,employeeEvalDetail.getInternalSupervisorNotes(),alternateRow);
