...
public class EDI834BCBSImagine extends AEDI834 {
	
	private List<Employee> empsDone = new ArrayList<Employee>();


	public EDI834BCBSImagine(String fileType)
	{
		super();
		this.fileType = fileType;
		ediLog = new EDILog("BCBS_EDI");
		vendorCode = "BCBS OF MI";
		logger = new ArahantLogger(EDI834BCBSImagine.class);
	}
	...
	public String[] dumpEDI(CompanyBase vendor, BEDITransaction tran, boolean debug) {
        ...
        List<HrBenefit> benefitsToDo = hsu.createCriteria(HrBenefit.class).eq(HrBenefit.BENEFIT_PROVIDER, vendor)
		...
		try {
			filename = new BProperty("LargeReportDir").getValue() + File.separator + bv.getAccountNumber() /*+ "EDI" + vendor.getInterchangeReceiverId().replace('-', '_') + "_" + icn + "_" + DateUtils.now() + "_" + (filenames.size() + 1)*/ + ".edi.txt";
        ...
        while (hscr.next()) {
			...
			if(!bj.getPayingPersonId().equals(currentSubscriber)) {
				...
				for(HrBenefitJoinH h : depJoins) {
					if(empsDone.contains(new BEmployee(h.getPayingPersonId()).getEmployee()))
						break;
					...
					if(!medicareType.equals("D") && !medicareType.equals("E"))
						groupAccountId = groupAccountId.substring(0, 5).concat("M").concat(groupAccountId.substring(6));
					ediMemberLevelDetail();
					if(h.getCoveredPersonId().equals(h.getPayingPersonId()))
						empsDone.add(new BEmployee(h.getPayingPersonId()).getEmployee());
				}
				depJoins.clear();
				flushData();
			}
        ...
        while (hscr.next()) {
			...
			if(!bj.getPayingPersonId().equals(currentSubscriber)) {
				...
				for(HrBenefitJoin j : depJoins) {
					if(empsDone.contains(new BEmployee(j.getPayingPersonId()).getEmployee()))
						break;
					...
					if(!medicareType.equals("D") && !medicareType.equals("E"))
						groupAccountId = groupAccountId.substring(0, 5).concat("M").concat(groupAccountId.substring(6));
					ediMemberLevelDetail();
					if(j.getCoveredPersonId().equals(j.getPayingPersonId()))
						empsDone.add(new BEmployee(j.getPayingPersonId()).getEmployee());
				}
				depJoins.clear();
			}
			depJoins.add(bj);
...
//            DTP_benefit_end_date();
	...
	private boolean INS_insured_benefit_return() {
	...
	private void DTP_eligibility_start_date() {
	...
	//	'00001-0000073523';  //BCBS group ID for Imagine Schools

//		DelimitedFileReader fr = new DelimitedFileReader("/home/xichen/Desktop/ImagineSchools.csv");
//		List<HrBenefit> benes = ArahantSession.getHSU().createCriteria(HrBenefit.class)
//													   .eq(HrBenefit.BENEFIT_PROVIDER, new BVendorCompany("00001-0000073523").getBean())
//													   .list();
//		fr.nextLine();
//		while(fr.nextLine()) {
//			for(HrBenefit bene : benes) {
//				BHRBenefit b = new BHRBenefit(bene);
//				if(b.getGroupId().equals(fr.getString(0)))
//					if(b.getSubGroupId().equals(fr.getString(1))) {
//						System.out.print("update hr_benefit set group_id = \'" + fr.getString(2) + "\', sub_group_id = \'" + fr.getString(3) + "\', group_account_id = \'" + fr.getString(4) + "\' ");
//						System.out.println("where benefit_id = \'" + b.getBenefitId() + "\';");
//					}
//			}
//		}

		List<HrBenefit> vis = ArahantSession.getHSU().createCriteria(HrBenefit.class).joinTo(HrBenefit.BENEFIT_CATEGORY).eq(HrBenefitCategory.DESCRIPTION, "Vision").list();
		
		for(HrBenefit b : vis)
			System.out.println("update hr_benefit set group_account_id = \'0000LT54\' where benefit_id = \'" + b.getBenefitId() + "\';");
	}
}
