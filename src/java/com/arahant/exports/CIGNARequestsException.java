 ...
 *
 */
public class CIGNARequestsException {

	HibernateSessionUtil hsu;
	SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyyy");

	public String export(Date start, Date end) throws Exception {
		File csvFile = FileSystemUtils.createTempFile("CIGNAException", ".csv");
		...
		while (scr.next()) {
			...
			String empLname = bemp.getLastName().replaceAll("amp;", "").replaceAll("&apos;", "'");
			String empFname = bemp.getFirstName();
			int dob;
...
/*
			 * writeRecord(dfw, branchCode, effectiveDate, eligibleEmpSsn,
			 * eligibleEmpId, memberSsn, lname, fname, dob, gender,
			 * relationship, fullTimeStudent, street1, street2, city, state,
			 * zip, dateOfHire, salary, frequency, phoneHome, phoneWork,
			 * medicalCode, pcpCode, existingPatient, pclEndDate, dentalCode,
			 * dentalOfficeCode, dentalLateEntrantApplies,
			 * dentalWaitPeriodApplies, dentalOriginalEffectiveDate, visionCode,
			 * flexHealthCode, employerGoalAmount, employeeGoalAmount,
			 * medicalAutoClaimForward, dentalAutoClaimForward,
			 * pharmacyAutoClaimForward, flexDependentCode,
			 * depEmployerGoalAmount, depEmployeeGoalAmount, cobraEffectiveDate,
			 * hasOtherMedical, otherMedCarrier, otherMedEffectiveDate,
			 * medicareType, otherMedPolicyNumber, primaryCode,
			 * financialResponsibility, cancellationDate, currentBenefitOption,
			 * futureBenefitOption, changeDate);
			 */
			...
			for (int loop = 0; loop < depNodes.getLength(); loop++) {
				...
				String lname = DOMUtils.getString(depNode, "lastName").replaceAll("amp;", "").replaceAll("&apos;", "'");
				String fname = DOMUtils.getString(depNode, "firstName");
				...
				if (!gender.equals(empGender)) {
					dfw.writeField(empLname);
					dfw.writeField(empFname);
				 ...
				 * relationship, fullTimeStudent, street1, street2, city, state,
				 * zip, dateOfHire, salary, frequency, phoneHome, phoneWork,
				 * medicalCode, pcpCode, existingPatient, pclEndDate,
				 * dentalCode, dentalOfficeCode, dentalLateEntrantApplies,
				 * dentalWaitPeriodApplies, dentalOriginalEffectiveDate,
				 * visionCode, flexHealthCode, employerGoalAmount,
				 * employeeGoalAmount, medicalAutoClaimForward,
				 * dentalAutoClaimForward, pharmacyAutoClaimForward,
				 * flexDependentCode, depEmployerGoalAmount,
				 * depEmployeeGoalAmount, cobraEffectiveDate, hasOtherMedical,
				 * otherMedCarrier, otherMedEffectiveDate, medicareType,
				 * otherMedPolicyNumber, primaryCode, financialResponsibility,
				 * cancellationDate, currentBenefitOption, futureBenefitOption,
				 * changeDate);
				 */
			}
		}
			...
			if (benefit.getHrBenefitCategory().getBenefitType() == type) {
				...
				for (int enloop = 0; enloop < enrollees.getLength(); enloop++)
					if (DOMUtils.getString(enrollees.item(enloop), "ssn").equals(DOMUtils.getString(depNode, "ssn"))
							&& DOMUtils.getString(enrollees.item(enloop), "firstName").equals(DOMUtils.getString(depNode, "firstName"))
							&& DOMUtils.getString(enrollees.item(enloop), "middleName").equals(DOMUtils.getString(depNode, "middleName"))
							&& DOMUtils.getString(enrollees.item(enloop), "lastName").equals(DOMUtils.getString(depNode, "lastName")))
	...
	public static void main(String args[]) throws Exception {
		...
		CIGNARequestsException x = new CIGNARequestsException();
