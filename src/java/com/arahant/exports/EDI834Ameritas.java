...
public class EDI834Ameritas {
	
	private String fileType = "F";
	private static final ArahantLogger logger = new ArahantLogger(EDI834Ameritas.class);
	//File logFile = new File(FileSystemUtils.getWorkingDirectory() + File.separator + "Logfiles/AMERITAS_EDI_LOG.txt");  //TODO: put this somewhere for clients
	private EDILog ediLog = new EDILog("AMERITAS_EDI");
	private static String vendorCode = "Ameritas";
	private EDI edi;
	private Date lastDumpDate;
	private List<String> payingPersons = new LinkedList<String>();



	public EDI834Ameritas(String fileType) {
...
//            hasMedicare = ArahantSession.getHSU().createCriteria(Employee.class).notIn(Employee.MEDICARE, new String[]{"", "N"}).exists();
		...
		private void setCoverageLevel(IHrBenefitJoin bj) {
			...
			if ((bj.getHrBenefitConfig().getSpouseEmpOrChildren() == 'Y'
					|| bj.getHrBenefitConfig().getSpouseNonEmpOrChildren() == 'Y')
					&& bj.getHrBenefitConfig().getMaxChildren() == 1)
				coverageLevelCode = "E1D";

			if ((bj.getHrBenefitConfig().getSpouseEmpOrChildren() == 'Y'
					|| bj.getHrBenefitConfig().getSpouseNonEmpOrChildren() == 'Y')
					&& bj.getHrBenefitConfig().getMaxChildren() == 2)
				coverageLevelCode = "E2D";

			if (bj.getHrBenefitConfig().getChildren() == 'Y' && bj.getHrBenefitConfig().getMaxChildren() == 0)
				coverageLevelCode = "ECH";

			if ((bj.getHrBenefitConfig().getSpouseEmpOrChildren() == 'Y'
					|| bj.getHrBenefitConfig().getSpouseNonEmpOrChildren() == 'Y')
					&& bj.getHrBenefitConfig().getMaxChildren() == 0)
...
//                    code = "4";
		...
		private void INS_maintenance_reason_code() {
			if (hasSpouse) {
				edi.SEG("INS");
				edi.ST(4, "11");
				edi.ST(5, "S");
		...
		public void REF_subscriber_late_entrant() {
			edi.SEG("REF");
			edi.SF(1, "ZZ");
			edi.SF(2, "NA"); //LE for late entrant, NA for not... what determines this?
			edi.EOS();
		}
...
//                edi.ST(4, maritalStatusCode);
		...
		/*
		 * AG - Preventive Care/Wellness AH - 24 Hour Care AJ - Medicare Risk AK
		 * - Mental Health DCP - Dental Capitation DEN - Dental EPO - Exclusive
		 * Provider Organization FAC - Facility HE - Hearing HLT - Health HMO -
		 * Health Maintenance Organization LTC - Long-Term Care LTD - Long-Term
		 * Disability MM - Major Medical MOD - Mail Order Drug PDG -
		 * Prescription Drug POS - Point of Service PPO - Preferred Provider
		 * Organization PRA - Practitioners STD - Short-Term Disability UR -
		 * Utilization Review VIS - Vision
		 */
		public void HD_health_coverage() {
			...
			switch (benefitType) {
				...
				case HrBenefitCategory.VISION:
					edi.SF(3, "VIS");
					edi.SF(4, "01"); //01 case based id from Ameritas
					break;
...
//                        ediLog.write("\tA valid Home Phone Number" + "\n");
	...
	public String[] dumpEDI(CompanyBase vendor, BEDITransaction tran, boolean debug) {
		...
		for (HrBenefit bene : benefitsToDo) {
			String filename = "";
			try {
				filename = new BProperty("LargeReportDir").getValue() + File.separator + "williamson_vision.txt";
...
//            hsu.commitTransaction();
		...
		try {
			...
			Logger.getLogger(EDI834Ameritas.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
