...
public class EDI834EBC {

	public EDI834EBC(String fileType)
	{
		this.fileType = fileType;
	}

	private String fileType = "F";
    private static final ArahantLogger logger = new ArahantLogger(EDI834EBC.class);
    ...
    private EDILog ediLog = new EDILog("EBC_EDI");
    private static String vendorCode = "Employee Benefit Concepts Inc";
			...
			if(new BPerson(payingPerson).getOrgGroupAssociations().size() > 0)
			{
				...
				else
				{
					...
					try {
						...
						Logger.getLogger(EDI834EBC.class.getName()).log(Level.SEVERE, null, ex);
					}
				}
...
//				REF_1L_other(groupNumber);
		...
		public void sendCobraTerms(String beneName) {
			try {
				BEmployee be = new BEmployee(ArahantSession.getHSU().createCriteria(Employee.class).eq(Employee.SSN, subscriberIdentifier).first());

				fw.writeField(be.getFirstName());
				fw.writeField(be.getMiddleName());
				fw.writeField(be.getLastName());
				fw.writeField(be.getSsn());
				fw.writeField(employmentEndDate);
				fw.writeField(beneName);
				fw.writeField(memberFirstName);
				fw.writeField(memberMiddleName);
				fw.writeField(memberLastName);
				fw.writeField(depSSN);
				String address = be.getStreet();
				if(!isEmpty(be.getStreet2()))
					address += " " + be.getStreet2();
				fw.writeField(address);
				fw.writeField(be.getCity());
				fw.writeField(be.getState());
				fw.writeField(be.getZip());
				fw.writeField(changeReason);
				fw.writeField(changeReasonDescription);
				fw.endRecord();
			}
			catch (Exception ex) {}
		}

		public void sendAddChanges(String beneName, String addChange, int changeDate) {
			try {
				BEmployee be = new BEmployee(ArahantSession.getHSU().createCriteria(Employee.class).eq(Employee.SSN, subscriberIdentifier).first());

				fw2.writeField(be.getFirstName());
				fw2.writeField(be.getMiddleName());
				fw2.writeField(be.getLastName());
				fw2.writeField(be.getSsn());
				fw2.writeField(beneName);
				fw2.writeField(addChange);
				fw2.writeField(changeDate == 0 ? "" : String.valueOf(changeDate));
				fw2.writeField(memberFirstName);
				fw2.writeField(memberMiddleName);
				fw2.writeField(memberLastName);
				fw2.writeField(depSSN);
				String address = be.getStreet();
				if(!isEmpty(be.getStreet2()))
					address += " " + be.getStreet2();
				fw2.writeField(address);
				fw2.writeField(be.getCity());
				fw2.writeField(be.getState());
				fw2.writeField(be.getZip());
				fw2.writeField(changeReason);
				fw2.endRecord();
			}
			catch (Exception ex) {}
		}
    ...
    private EDI edi;
	private DelimitedFileWriter fw, fw2;
	...
	private void doCobra(HibernateSessionUtil hsu, Employee emp, List<HrBenefit> cobraBenes) {
        int count = 0;

        HibernateCriteriaUtil<HrBenefitJoinH> hcuInactives = hsu.createCriteria(HrBenefitJoinH.class);
        hcuInactives.orderBy(HrBenefitJoinH.PAYING_PERSON_ID);
		hcuInactives.eq(HrBenefitJoinH.PAYING_PERSON, emp);
        hcuInactives.leOrEq(HrBenefitJoinH.POLICY_END_DATE, DateUtils.now(), 0);
        hcuInactives.gt(HrBenefitJoinH.RECORD_CHANGE_DATE, lastDumpDate);
        hcuInactives.joinTo(HrBenefitJoinH.HR_BENEFIT_CONFIG).in(HrBenefitConfig.HR_BENEFIT, cobraBenes);
        hcuInactives.ne(HrBenefitJoinH.COVERAGE_START_DATE, 0);
		List<HrBenefitJoinH> cobraInactiveTerms = hcuInactives.list();

		HibernateCriteriaUtil<HrBenefitJoin> hcuActives = hsu.createCriteria(HrBenefitJoin.class).eq(HrBenefitJoin.BENEFIT_DECLINED, 'N').eq(HrBenefitJoin.APPROVED, 'Y');
        hcuActives.orderBy(HrBenefitJoin.POLICY_START_DATE);
        hcuActives.orderBy(HrBenefitJoin.PAYING_PERSON_ID);
		hcuActives.eq(HrBenefitJoin.PAYING_PERSON, emp);
        hcuActives.joinTo(HrBenefitJoin.HR_BENEFIT_CONFIG).in(HrBenefitConfig.HR_BENEFIT, cobraBenes);
        hcuActives.ltAndNeq(HrBenefitJoin.COVERAGE_START_DATE, DateUtils.now(), 0);
		List<HrBenefitJoin> cobraActiveTerms = hcuActives.list();

        List<EDIDumpPersonalInfo> batch = new LinkedList<EDIDumpPersonalInfo>();

		for(HrBenefitJoinH h : cobraInactiveTerms) {
			EDIDumpPersonalInfo info = new EDIDumpPersonalInfo(fw);
			...
			try {
				...
				int benefitType = h.getHrBenefitConfig().getHrBenefit().getHrBenefitCategory().getBenefitType();
				info.setBenefitJoinData(h, benefitType);
		...
		Collections.sort(batch);
		for (EDIDumpPersonalInfo i : batch)
			i.sendCobraTerms(i.benefitName);
		batch.clear();

		for(HrBenefitJoin j : cobraActiveTerms) {
			EDIDumpPersonalInfo info = new EDIDumpPersonalInfo(fw);
			...
			int benefitType = j.getHrBenefitConfig().getHrBenefit().getHrBenefitCategory().getBenefitType();
			info.setBenefitJoinData(j, benefitType);

			if (j.getCoverageStartDate() != 0 && j.getRecordChangeDate().after(lastDumpDate))
				info.maintenanceTypeCode = "021";
			else if (j.getCoverageEndDate() != 0 && j.getRecordChangeDate().after(lastDumpDate))
                info.maintenanceTypeCode = "024";
			else if (j.getPolicyEndDate() != 0 && j.getRecordChangeDate().after(lastDumpDate))
                info.maintenanceTypeCode = "024";
			else if (j.getRecordChangeDate().after(lastDumpDate))
				info.maintenanceTypeCode = "001";
			else if (j.getCoverageEndDate() != 0 && j.getRecordChangeDate().before(lastDumpDate))
                continue;
            if (j.getPolicyEndDate() != 0 && j.getRecordChangeDate().before(lastDumpDate))
                continue;

			info.recordChangeDate = DateUtils.getDate(j.getRecordChangeDate());
			batch.add(info);
		}
		Collections.sort(batch);
		for (EDIDumpPersonalInfo i : batch)
	...
	private void doAddChanges(HibernateSessionUtil hsu, List<HrBenefit> cobraBenes) {
        int count = 0;

		HibernateCriteriaUtil<HrBenefitJoin> hcuAdds = hsu.createCriteria(HrBenefitJoin.class).eq(HrBenefitJoin.BENEFIT_DECLINED, 'N').eq(HrBenefitJoin.APPROVED, 'Y');
        hcuAdds.orderBy(HrBenefitJoin.POLICY_START_DATE);
        hcuAdds.orderBy(HrBenefitJoin.PAYING_PERSON_ID);
        hcuAdds.joinTo(HrBenefitJoin.HR_BENEFIT_CONFIG).in(HrBenefitConfig.HR_BENEFIT, cobraBenes);
        hcuAdds.geOrEq(HrBenefitJoin.COVERAGE_END_DATE, DateUtils.now(), 0);
        hcuAdds.geOrEq(HrBenefitJoin.COVERAGE_START_DATE, DateUtils.getDate(lastDumpDate), 0);
		List<HrBenefitJoin> adds = hcuAdds.list();

		HibernateCriteriaUtil<HrBenefitJoin> hcuChanges = hsu.createCriteria(HrBenefitJoin.class).eq(HrBenefitJoin.BENEFIT_DECLINED, 'N').eq(HrBenefitJoin.APPROVED, 'Y');
        hcuChanges.joinTo(HrBenefitJoin.HR_BENEFIT_CONFIG).in(HrBenefitConfig.HR_BENEFIT, cobraBenes);
        hcuChanges.ltAndNeq(HrBenefitJoin.COVERAGE_START_DATE, DateUtils.now(), 0);
        hcuChanges.geOrEq(HrBenefitJoin.COVERAGE_END_DATE, DateUtils.now(), 0);
		hcuChanges.gt(HrBenefitJoin.RECORD_CHANGE_DATE, lastDumpDate);
		hcuChanges.orderBy(HrBenefitJoin.RECORD_CHANGE_DATE);
		List<HrBenefitJoin> changes = hcuChanges.list();

        List<EDIDumpPersonalInfo> batch = new LinkedList<EDIDumpPersonalInfo>();

		for(HrBenefitJoin j : adds) {
			EDIDumpPersonalInfo info = new EDIDumpPersonalInfo(fw2);
		...
		Collections.sort(batch);
		for (EDIDumpPersonalInfo i : batch)
			i.sendAddChanges(i.benefitName, "Add", i.recordChangeDate);
		batch.clear();
		
		for(HrBenefitJoin j : changes) {
			EDIDumpPersonalInfo info = new EDIDumpPersonalInfo(fw);
		...
		Collections.sort(batch);
		for (EDIDumpPersonalInfo i : batch)
			i.sendAddChanges(i.benefitName, "Change", i.recordChangeDate);
		batch.clear();
    }

	public void sendCobraTermsHeader(DelimitedFileWriter fw) {
		try {
			fw.writeField("Employee First Name");
			fw.writeField("Employee Middle Name");
			fw.writeField("Employee Last Name");
			fw.writeField("Employee SSN");
			fw.writeField("Term Date");
			fw.writeField("Benefit");
			fw.writeField("Covered Person First Name");
			fw.writeField("Covered Person Middle Name");
			fw.writeField("Covered Person Last Name");
			fw.writeField("Covered Person SSN");
			fw.writeField("Employee Street Address");
			fw.writeField("Employee City");
			fw.writeField("Employee State");
			fw.writeField("Employee Zip");
			fw.writeField("Termination Condition");
			fw.writeField("Termination Notes");
			fw.endRecord();
		}
		catch (Exception ex) {}
	}
	
	public void sendAddChangesHeader(DelimitedFileWriter fw) {
		try {
			fw.writeField("Employee First Name");
			fw.writeField("Employee Middle Name");
			fw.writeField("Employee Last Name");
			fw.writeField("Employee SSN");
			fw.writeField("Benefit");
			fw.writeField("Add/Change");
			fw.writeField("Add/Change Date");
			fw.writeField("Covered Person First Name");
			fw.writeField("Covered Person Middle Name");
			fw.writeField("Covered Person Last Name");
			fw.writeField("Covered Person SSN");
